...
nconf.argv({ parseValues: true })
    ...
    .required([ 'rethink_database', 'rethink_host', 'rethink_port', 'crate_host', 'crate_port', 'app_port' ])

// Connect to databases
const r = rethinkdbdash({
    db: nconf.get( 'rethink_database' ),
    servers: [ { host: nconf.get( 'rethink_host' ), port: nconf.get( 'rethink_port' ) } ]
});
...
router.get( '/logs/cratedb', async ctx => {
    ...
    const entries = await r.table( 'logs' ).filter( x => x( 'time' ).between( minDate, maxDate ) ).run();
...
router.get( '/logs/rethinkdb', async ctx => {
    ...
    const entries = await crate.execute( 'SELECT * FROM logs WHERE time BETWEEN ? AND ?', [ minDate, maxDate ]);
